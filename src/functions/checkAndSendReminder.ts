...
export const checkAndSendReminder = async () => {
	...
	const generatedDatetime = fixZero(new Date().toLocaleString('pt-BR'))
		.substr(0, 16)
		.split(' ')

	// Adequa a data para o formato guardado no banco de dados
	const datetime = `${generatedDatetime[0].split('/')[2]}-${
		generatedDatetime[0].split('/')[1]
	}-${generatedDatetime[0].split('/')[0]} ${generatedDatetime[1]}`
